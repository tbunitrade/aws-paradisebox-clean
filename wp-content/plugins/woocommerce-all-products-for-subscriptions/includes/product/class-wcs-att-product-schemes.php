				...
				if ( 'yes' !== $disable_schemes ) {
					...
					if ( false === $schemes_data && $product->is_type( 'variation' ) ) {
						...
						if ( ! is_a( $parent, 'WC_Product' ) ) {
							$message = sprintf( __( 'Could not fetch subscription plans for variation with ID %d. This indicates that this variation may be an orphaned variation. To clear orphaned variations, go to "WooCommerce > Status > Tools" and click "Delete orphaned variations".', 'woocommerce-all-products-for-subscriptions' ), $product->get_id() );
