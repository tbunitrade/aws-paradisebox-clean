	...
	*/
	...
	public static function get_price_html( $product, $scheme_key = '', $args = array() ) {
		...
		if ( $scheme_switch_required && false === $switched_scheme ) {
			...
			$subscribe_discounted_html = ! empty( $args['subscribe_discounted_html'] ) ? $args['subscribe_discounted_html'] : _x( 'Subscribe to save %1$s%2$s', 'Subscribe to plan(s) for discount', 'woocommerce-all-products-for-subscriptions' );
			...
			if ( WCS_ATT_Product::is_subscription( $product ) ) {
				...
				if ( WCS_ATT_Product_Schemes::has_forced_subscription_scheme( $product ) ) {
					...
					if ( $apply_discount_price_html_suffix ) {
						...
						if ( 'prompt' === $context ) {
							/* translators: %s: Discount % (Use encoded value when translating the % character. Use &#37; instead of %.) */
							$discount_html = ' <span class="wcsatt-sub-discount">' . sprintf( _x( '%s&#37;', 'subscribe to save discount', 'woocommerce-all-products-for-subscriptions' ), round( $base_scheme_discount, self::get_formatted_discount_precision() ) ) . '</span>';
							...
							/* translators: %s: Discount % (Use encoded value when translating the % character. Use &#37; instead of %.) */
							$discount_html = '</small> <span class="wcsatt-sub-discount">' . sprintf( _x( '%s&#37;', 'subscribe to save discount', 'woocommerce-all-products-for-subscriptions' ), round( $base_scheme_discount, self::get_formatted_discount_precision() ) ) . '</span><small>';
							/* translators: %1$s: "up to" text or empty string, %2$s: Discount % with HTML (Use encoded value when translating the % character. Use &#37; instead of %.) */
							$suffix_price_html = sprintf( __( 'subscribe to save %1$s%2$s', 'woocommerce-all-products-for-subscriptions' ), $has_variable_discount ? __( 'up to', 'woocommerce-all-products-for-subscriptions' ) : '', $discount_html );
							/* translators: %s: subscribe to save suffix */
							$suffix = '<small class="wcsatt-sub-options">' . sprintf( _x( ' <span class="wcsatt-dash">&mdash;</span> or %s', 'subscribe to save suffix format', 'woocommerce-all-products-for-subscriptions' ), $suffix_price_html ) . '</small>';
						}
					} else {
