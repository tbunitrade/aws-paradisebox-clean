 ...
 *
 * Please note that the plugin/package name is "WooCommerce All Product for Subscriptions"
 * (so that all WooCommerce plugins are grouped together in the Plugins screen),
 * but the product name is "All Products for WooCommerce Subscriptions".
 */
...
if ( ! class_exists( 'WCS_ATT' ) ) :
	...
	class WCS_ATT extends WCS_ATT_Abstract_Module {
			...
			if ( class_exists( 'WC_Subscriptions' ) ) {
				...
				if ( ! defined( 'WCS_INIT_TIMESTAMP' ) || ! WC_Payments_Features::is_wcpay_subscriptions_enabled() ) {
					if ( version_compare( WCPAY_VERSION_NUMBER, self::REQ_WCPAY_VERSION ) < 0 ) {
						...
						$notice = sprintf( __( 'All Products for WooCommerce Subscriptions requires at least <a href="%1$s" target="_blank">WooPayments</a> version <strong>%2$s</strong>.', 'woocommerce-all-products-for-subscriptions' ), self::get_resource_url( 'wcpay' ), self::REQ_WCPAY_VERSION );
					} else {
						// Notice about disabled Subscription features in WooPayments.
						$notice = sprintf( __( 'All Products for WooCommerce Subscriptions requires Subscriptions to be enabled in the <strong>WooPayments</strong> <a href="%1$s" target="_blank">settings</a>.', 'woocommerce-all-products-for-subscriptions' ), self::get_resource_url( 'wcpay-settings' ) );
					}
				}
			} else {
				// Notice about disabled Subscriptions core not being loaded.
				$notice = sprintf( __( 'All Products for WooCommerce Subscriptions requires at least <a href="%1$s" target="_blank">WooPayments</a> version <strong>%2$s</strong> or <a href="%3$s" target="_blank">WooCommerce Subscriptions</a> version <strong>%4$s</strong>.', 'woocommerce-all-products-for-subscriptions' ), self::get_resource_url( 'wcpay' ), self::REQ_WCPAY_VERSION, self::get_resource_url( 'wcs' ), self::REQ_WCS_VERSION );
			}
		...
		public function includes() {
			...
			// Data modules
			require_once WCS_ATT_ABSPATH . 'includes/class-wcs-att-product-data.php';
